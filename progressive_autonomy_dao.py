...
class ProgressiveAutonomyDAO(gl.Contract):
    ...
    @gl.public.write
    def add_member(self, new_member: Address) -> None:
    ...
    def vote(self, proposal_id: u256, support: bool) -> None:
        ...
        self.voted_log.append(f"{int(proposal_id)}:{str(sender)}")
    ...
    def _is_member(self, addr: Address) -> bool:
        for i in range(len(self.members)):
            if self.members[i] == addr:
                return True
        return False

    def _has_voted(self, proposal_id: u256, addr: Address) -> bool:
        key = f"{int(proposal_id)}:{str(addr)}"
